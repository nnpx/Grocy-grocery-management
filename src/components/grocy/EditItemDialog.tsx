...
export function EditItemDialog({ isOpen, onClose, onEditItem, item, categories }: EditItemDialogProps) {
  ...
  useEffect(() => {
    if (item) {
      ...
      setExpiryDate(parseISO(item.expiryDate));
    }
  }, [item]);

  const handleSave = () => {
    if (name && category && expiryDate && item) {
      onEditItem({
        ...item,
        name,
        category,
